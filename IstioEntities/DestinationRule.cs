...
public class DestinationRule() : CustomResource<DestinationRuleSpec>
    ("DestinationRule", $"{GROUP}/{VERSION}", new DestinationRuleSpec())
{
...
public class DestinationRuleSpec
{
    ...
    ///     To avoid potential misconfigurations, it is recommended to always use fully qualified domain names over short
    ///     names.
    ...
    /// <summary>
    ///     One or more named sets that represent individual versions of a service.Traffic policies can be overridden at subset
    ///     level.
    /// </summary>
    ...
    ///     If specified, the DestinationRule configuration will be applied only to the workload instances matching the
    ///     workload selector label in the same namespace.
    ...
    ///     This feature provides a mechanism for service owners and mesh administrators to control the visibility of
    ///     destination rules across namespace boundaries.
    /// </summary>
...
/// <summary>
///     A subset of endpoints of a service. Subsets can be used for scenarios like A/B testing, or routing to a specific
...
///     The following rule uses a round robin load balancing policy for all traffic going to a subset named testversion
///     that is composed of endpoints (e.g., pods) with labels (version:v3).
/// </summary>
[Serializable]
public class Subset
{
